...
def _config_report() -> dict:
    ...
    return {
        ...
        # Meta block. Absent until now, which is why /debug/config could say
        # nothing about the channel that actually serves customers.
        "meta_phone_number_id": settings.meta_phone_number_id or "UNSET",
        "meta_verify_token": _fingerprint(settings.meta_webhook_verify_token),
        "meta_access_token": _fingerprint(settings.meta_access_token),
        "meta_app_secret": _fingerprint(settings.meta_app_secret),
